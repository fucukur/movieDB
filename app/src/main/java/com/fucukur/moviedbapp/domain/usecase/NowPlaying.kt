...
class NowPlaying @Inject constructor(private val repo: MovieRepoInterface) {
    operator fun invoke(): Flow<Result<List<NetworkMovie>>> = flow {
        try {
            ...
            emit(Result.Error(message = e.localizedMessage ?: "End Connection Error Models. test fucukur"))
        }
    }
}